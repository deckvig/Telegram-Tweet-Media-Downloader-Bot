...
func download(message string) error {
	url := getUrl(message) // Extract the URL from the message

	// Get the proxy from environment variables
	proxyURL := os.Getenv("HTTP_PROXY")
	if proxyURL == "" {
		// If no proxy is set, return an error or handle accordingly
		return fmt.Errorf("HTTP_PROXY environment variable is not set")
	}

	// Construct the command
	// Note: We use the full command path for robustness. You may need to adjust this.
	// For example: `exec.Command("/usr/local/bin/gallery-dl", "--proxy", proxyURL, url)`
	cmd := exec.Command("gallery-dl", "--proxy", proxyURL, url)
